...
let csrfToken = null; // CSRF Protection Token
...
const elFormForgot = document.getElementById("formForgot");
const elFormReset = document.getElementById("formReset");
const elAuthFooter = document.getElementById("authFooter");
let recoveryData = { username: "", email: "", code: "" };
...
function showOfflineToast(url, options) {
    const container = document.getElementById("toastContainer");
    if (!container) return;
    ...
    toast.innerHTML = `
        <div class="toast-message">
            Offline: Unable to reach the server. 
            <a href="#" id="toastRetryLink" style="color: var(--indigo); font-weight: 700; text-decoration: underline; margin-left: 8px;">Retry Connection</a>
        </div>
        <button class="toast-close">&times;</button>
    `;
    ...
    toast.querySelector("#toastRetryLink").addEventListener("click", async (e) => {
        e.preventDefault();
        dismiss();
        showToast("Retrying action...", "info");
        try {
            await apiRequest(url, options);
            showToast("Success! Action retried successfully.", "success");
...
// -------------------------------------------------------------
// Button Loading State Toggler (Double Click Prevention)
// -------------------------------------------------------------
function setButtonLoading(button, isLoading, loadingText) {
    ...
    if (isLoading) {
        ...
        button.innerHTML = `<span class="spinner-inline"></span><span>${loadingText}</span>`;
...
function setAuthMode(mode) {
    ...
    // Hide the recovery forms by default
    if (elFormForgot) elFormForgot.style.display = "none";
    if (elFormReset) elFormReset.style.display = "none";
    if (elFormAuth) elFormAuth.style.display = "block";
    if (elAuthFooter) elAuthFooter.style.display = "flex";
    ...
    if (authMode === 'login') {
        ...
        if (elFormAuth) elFormAuth.style.display = "none";
        if (elAuthFooter) elAuthFooter.style.display = "none";
        if (elFormForgot) elFormForgot.style.display = "block";
        document.querySelector(".auth-header h2").textContent = "Recover Account";
        document.getElementById("authSubtitle").textContent = "Reset your account password.";
    } else if (authMode === 'reset') {
        if (elFormAuth) elFormAuth.style.display = "none";
        if (elAuthFooter) elAuthFooter.style.display = "none";
        if (elFormReset) elFormReset.style.display = "block";
        document.querySelector(".auth-header h2").textContent = "Reset Password";
        document.getElementById("authSubtitle").textContent = "Create a new password.";
    }
}
...
async function checkAuthStatus() {
    try {
        ...
        if (status.authenticated) {
            ...
            const appContainer = document.getElementById('appContainer');
            if (document.startViewTransition && (!appContainer || appContainer.style.display === 'none')) {
            ...
            // Not authenticated, landing page is visible by default
            const lp = document.getElementById("landingPage");
            if(lp && lp.style.display === "none") {
                showAuthBackdrop();
            }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (landingTitle) {
        ...
        let i = 0;
        let isMasterDSA = false;
        function typeWriter() {
            if (i < text.length) {
                if (text.charAt(i) === '\n') {
                    landingTitle.innerHTML += "<br>";
                } else {
                    if (text.substring(i).startsWith("Master DSA.")) {
                        isMasterDSA = true;
                    }
                    if (isMasterDSA) {
                        // Check if we just started the styled text
                        if (text.substring(i).startsWith("Master DSA.")) {
                            landingTitle.innerHTML += "<span style='color: var(--indigo)'>" + text.charAt(i);
                        } else if (text.charAt(i) === '.' && i === text.length - 1) {
                            landingTitle.innerHTML += text.charAt(i) + "</span>";
                        } else {
                            landingTitle.innerHTML += text.charAt(i);
                        }
                    } else {
                        landingTitle.innerHTML += text.charAt(i);
                    }
...
function updateStudyPlans() {
    ...
    state.plans.forEach(plan => {
        ...
        planCard.querySelector(".plan-delete-btn").addEventListener("click", (e) => {
            e.stopPropagation();
            showDeleteConfirmation(`Are you sure you want to delete the plan "${plan.title}"?`, async () => {
...
async function deletePlan(planId) {
    try {
        ...
        showToast("Study plan soft-deleted successfully.", "success");
        await fetchAllUserData();
    } catch (err) {
...
function updateProblemsLogTable() {
    ...
    filtered.forEach(p => {
        ...
        tr.querySelector(".delete").addEventListener("click", () => {
            showDeleteConfirmation(`Are you sure you want to delete the problem log for "${p.name}"?`, async () => {
...
async function deleteProblem(id) {
    try {
        ...
        showToast("Problem soft-deleted successfully.", "success");
        await fetchAllUserData();
    } catch (err) {
        showToast(err.message, "error");
    }
...
function editProblem(id) {
    ...
    // Solution attachment state restoration
    document.getElementById("uploadFileName").textContent = "No file chosen";
    document.getElementById("btnUploadFile").style.display = "none";
    if (p.attachment_url) {
        document.getElementById("problemAttachmentUrl").value = p.attachment_url;
        document.getElementById("attachmentPreviewContainer").style.display = "block";
        document.getElementById("attachmentPreviewLink").href = p.attachment_url;
    } else {
        document.getElementById("problemAttachmentUrl").value = "";
        document.getElementById("attachmentPreviewContainer").style.display = "none";
    }
...
function setupEventListeners() {
    ...
    if (btnLandingLogin) {
        btnLandingLogin.addEventListener("click", (e) => {
            if (!document.startViewTransition) {
    ...
    const btnLandingGetStarted = document.getElementById("btnLandingGetStarted");
    if (btnLandingGetStarted) {
        btnLandingGetStarted.addEventListener("click", () => {
            if (btnLandingLogin) btnLandingLogin.click();
        });
    }
    ...
    if (btnLandingTheme) btnLandingTheme.addEventListener("click", themeHandler);

    const btnAuthTheme = document.getElementById("btnAuthTheme");
    if (btnAuthTheme) btnAuthTheme.addEventListener("click", themeHandler);
    ...
    if (elBtnSendSignupCode) {
        elBtnSendSignupCode.addEventListener("click", async () => {
            ...
            const target = email;
            if (!target) {
                showToast("Please enter an email address to receive the verification code.", "error");
                return;
            }
            
            setButtonLoading(elBtnSendSignupCode, true, "Sending...");
            try {
                ...
                setButtonLoading(elBtnSendSignupCode, false);
                elBtnSendSignupCode.disabled = true;
                
                if (res && res.sent_real) {
                    showToast(`Verification code sent to ${target}. Please check your inbox!`, "success");
                } else {
                    showToast(`[Dev Mode] Verification code printed to the server terminal console log.`, "info");
                }
    ...
    elFormAuth.addEventListener("submit", async (e) => {
        ...
        const btnText = authMode === 'login' ? "Logging in..." : "Creating Account...";
        setButtonLoading(elBtnAuthSubmit, true, btnText);
        try {
            ...
            setButtonLoading(elBtnAuthSubmit, false, authMode === 'login' ? "Log In" : "Create Account");
        }
    });
    ...
    elBtnLogOut.addEventListener("click", async () => {
        if (confirm("Are you sure you want to log out?")) {
            setButtonLoading(elBtnLogOut, true, "Logging out...");
            try {
                ...
                setButtonLoading(elBtnLogOut, false, "Log Out");
            }
        }
    });

    // Logout from all devices Button
    const elBtnProfileLogoutAll = document.getElementById("btnProfileLogoutAll");
    if (elBtnProfileLogoutAll) {
        elBtnProfileLogoutAll.addEventListener("click", async () => {
            if (confirm("Are you sure you want to log out from all devices? All other sessions will be invalidated.")) {
                setButtonLoading(elBtnProfileLogoutAll, true, "Logging out all...");
                try {
                    await apiRequest('/api/auth/logout-all', { method: 'POST' });
                    showToast("Invalidated all other sessions successfully.", "success");
                    ...
                    setButtonLoading(elBtnProfileLogoutAll, false, "Logout All Devices");
                }
            }
        });
    }
    ...
    if (elBtnSendResetCode) {
        elBtnSendResetCode.addEventListener("click", async () => {
            ...
            setButtonLoading(elBtnSendResetCode, true, "Sending...");
            try {
                const res = await apiRequest('/api/auth/send-code', {
                    method: 'POST',
                ...
                setButtonLoading(elBtnSendResetCode, false);
    ...
    if (elFormResetPassword) {
        elFormResetPassword.addEventListener("submit", async (e) => {
            ...
            const submitBtn = elFormResetPassword.querySelector("button[type='submit']");
    ...
    elFormSettings.addEventListener("submit", async (e) => {
        ...
        const submitBtn = elFormSettings.querySelector("button[type='submit']");
    ...
    elBtnResetProgress.addEventListener("click", async () => {
        ...
        showDeleteConfirmation(msg, async () => {
            setButtonLoading(elBtnResetProgress, true, "Resetting...");
            try {
                ...
                setButtonLoading(elBtnResetProgress, false, "Reset Progress");
            }
        });
    });

    // File upload trigger button inside Log Problem Modal
    const fileInput = document.getElementById("problemAttachment");
    const btnTriggerUpload = document.getElementById("btnTriggerUpload");
    const uploadFileName = document.getElementById("uploadFileName");
    const btnUploadFile = document.getElementById("btnUploadFile");
    const hiddenUrlInput = document.getElementById("problemAttachmentUrl");
    ...
    if (fileInput) {
        fileInput.addEventListener("change", () => {
            ...
            if (file) {
                ...
                btnUploadFile.style.display = "inline-flex";
            } else {
    ...
    if (btnUploadFile) {
        btnUploadFile.addEventListener("click", async () => {
            ...
            setButtonLoading(btnUploadFile, true, "Uploading...");
            ...
            try {
                ...
                hiddenUrlInput.value = res.url;
                showToast("Solution attachment uploaded successfully!", "success");
                btnUploadFile.style.display = "none";
                ...
                setButtonLoading(btnUploadFile, false, "Upload File");
            }
        });
    }
    ...
    elFormLogProblem.addEventListener("submit", async (e) => {
        ...
        const attachment_url = hiddenUrlInput.value;
        ...
        try {
            ...
            showToast(pid ? "Problem updated successfully!" : "New problem logged successfully!", "success");
            ...
            setButtonLoading(saveBtn, false, "Save Problem");
        }
    });
    ...
    elFormCreatePlan.addEventListener("submit", async (e) => {
        ...
        const submitBtn = elFormCreatePlan.querySelector("button[type='submit']");
        ...
        try {
            ...
            setButtonLoading(submitBtn, false, "Create Plan");
        }
    });
    ...
    if (elBtnBulkDelete) {
        elBtnBulkDelete.addEventListener("click", async () => {
            ...
            showDeleteConfirmation(`Are you sure you want to bulk delete the ${idsToDelete.length} selected problems?`, async () => {
                setButtonLoading(elBtnBulkDelete, true, "Deleting...");
                try {
                    await apiRequest('/api/problems/bulk-delete', {
                        method: 'POST',
                        body: JSON.stringify({ ids: idsToDelete })
                    });
                    showToast(`Successfully soft-deleted ${idsToDelete.length} problems.`, "success");
                    await fetchAllUserData();
                } catch (err) {
                    showToast(err.message, "error");
                } finally {
                    setButtonLoading(elBtnBulkDelete, false, `Delete Selected (<span id="bulkDeleteCount">0</span>)`);
                    updateBulkDeleteButtonState();
                }
            });
        });
    ...
    // -------------------------------------------------------------
    // Forgot / Reset Password (Step-by-Step Inline flow)
    // -------------------------------------------------------------
    const elBtnSendForgotCode = document.getElementById("btnSendForgotCode");
    let forgotTimer = null;
    if (elBtnSendForgotCode) {
        elBtnForgotNext.disabled = false;
        elBtnSendForgotCode.addEventListener("click", async () => {
            ...
            setButtonLoading(elBtnSendForgotCode, true, "Sending...");
            try {
                const res = await apiRequest('/api/auth/send-code', {
                    method: 'POST',
                    body: JSON.stringify({ username, target: email, purpose: 'reset' })
                });
                
                setButtonLoading(elBtnSendForgotCode, false);
                elBtnSendForgotCode.disabled = true;
                ...
                let secondsLeft = 30;
                elBtnSendForgotCode.textContent = `Resend in ${secondsLeft}s`;
                if (forgotTimer) clearInterval(forgotTimer);
                forgotTimer = setInterval(() => {
                    secondsLeft--;
                    if (secondsLeft <= 0) {
                        clearInterval(forgotTimer);
                        elBtnSendForgotCode.disabled = false;
                        elBtnSendForgotCode.textContent = "Resend Code";
                    } else {
                        elBtnSendForgotCode.disabled = true;
                        elBtnSendForgotCode.textContent = `Resend in ${secondsLeft}s`;
                    }
                ...
                setButtonLoading(elBtnSendForgotCode, false);
                elBtnSendForgotCode.textContent = "Send Code";
            }
        });
    }
    ...
    if (formForgot) {
        formForgot.addEventListener("submit", async (e) => {
            ...
            const errorEl = document.getElementById("forgotErrorMsg");
            const submitBtn = document.getElementById("btnForgotNext");
            ...
            try {
                await apiRequest('/api/auth/verify-reset-code', {
                    method: 'POST',
                    body: JSON.stringify({ username, verification: email, code })
                });
                
                recoveryData = { username, email, code };
            ...
            try {
                await apiRequest('/api/auth/reset-password', {
                    ...
                    body: JSON.stringify({
                        ...
                        verification: recoveryData.email,
    ...
    const btnToggleResetNew = document.getElementById("btnToggleResetPasswordNew");
    if (btnToggleResetNew) {
        btnToggleResetNew.addEventListener("click", () => {
            const input = document.getElementById("resetPasswordNewInput");
            const type = input.getAttribute("type") === "password" ? "text" : "password";
            input.setAttribute("type", type);
            const eyeOpen = btnToggleResetNew.querySelector(".eye-open");
            const eyeClosed = btnToggleResetNew.querySelector(".eye-closed");
            if (type === "password") {
                if (eyeOpen) eyeOpen.style.display = "block";
                if (eyeClosed) eyeClosed.style.display = "none";
            } else {
    ...
    const btnToggleResetConfirm = document.getElementById("btnToggleResetPasswordConfirm");
    if (btnToggleResetConfirm) {
        btnToggleResetConfirm.addEventListener("click", () => {
            const input = document.getElementById("resetPasswordConfirmInput");
            const type = input.getAttribute("type") === "password" ? "text" : "password";
            input.setAttribute("type", type);
            const eyeOpen = btnToggleResetConfirm.querySelector(".eye-open");
            const eyeClosed = btnToggleResetConfirm.querySelector(".eye-closed");
            if (type === "password") {
                if (eyeOpen) eyeOpen.style.display = "block";
                if (eyeClosed) eyeClosed.style.display = "none";
            } else {
    ...
    const resetNewInput = document.getElementById("resetPasswordNewInput");
    const resetStrengthWrapper = document.getElementById("forgotResetPasswordStrengthWrapper");
    const resetStrengthBar = document.getElementById("forgotResetPasswordStrengthBar");
    const resetStrengthText = document.getElementById("forgotResetPasswordStrengthText");
    if (resetNewInput) {
        resetNewInput.addEventListener("input", () => {
            const val = resetNewInput.value;
            if (!val) {
            ...
            const strength = checkPasswordStrength(val);
            resetStrengthText.textContent = strength;
            if (strength === "Weak") {
                resetStrengthBar.style.width = "33%";
                resetStrengthBar.style.backgroundColor = "var(--rose)";
                resetStrengthText.style.color = "var(--rose)";
            } else if (strength === "Medium") {
                resetStrengthBar.style.width = "66%";
                resetStrengthBar.style.backgroundColor = "var(--amber)";
                resetStrengthText.style.color = "var(--amber)";
            } else {
                resetStrengthBar.style.width = "100%";
                resetStrengthBar.style.backgroundColor = "var(--emerald)";
                resetStrengthText.style.color = "var(--emerald)";
            }
        });
    }
    ...
    window.addEventListener("keydown", (e) => {
        if (e.key === "Escape") {
            ...
            closeModal(elModalSettings);
            closeModal(elModalResetPassword);
            closeModal(document.getElementById("modalConfirmDelete"));
        }
    });
}
...
// -------------------------------------------------------------
// Trash Bin Loader & Event Bindings
// -------------------------------------------------------------
async function loadTrashData() {
    try {
        const data = await apiRequest('/api/trash');
        const probBody = document.getElementById("trashProblemsBody");
        const planBody = document.getElementById("trashPlansBody");
        const probEmpty = document.getElementById("trashProblemsEmpty");
        const planEmpty = document.getElementById("trashPlansEmpty");
        ...
        // Show/hide admin column headers
        const adminCols = document.querySelectorAll(".admin-only-col");
        adminCols.forEach(col => {
            col.style.display = isAdmin ? "table-cell" : "none";
        });
        
        // 1. Render deleted problems
        if (!data.problems || data.problems.length === 0) {
            probEmpty.style.display = "block";
            document.getElementById("trashProblemsTable").style.display = "none";
        } else {
            probEmpty.style.display = "none";
            document.getElementById("trashProblemsTable").style.display = "table";
            
            data.problems.forEach(p => {
                const tr = document.createElement("tr");
                const delDate = new Date(p.deleted_at).toLocaleDateString(undefined, {
                    month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit'
                });
                
                let ownerHtml = "";
                if (isAdmin) {
                    ownerHtml = `<td><span class="small-text-gray">${p.owner_username || 'Unknown'}</span></td>`;
                }
                
                let actionHtml = `
                    <td>
                        <div class="actions-cell">
                            <button class="btn btn-secondary btn-sm btn-restore-prob" data-id="${p.id}" style="padding: 4px 8px; font-size: 0.8rem;">Restore</button>
                            ${isAdmin ? `<button class="btn btn-primary btn-sm btn-purge-prob" data-id="${p.id}" style="padding: 4px 8px; font-size: 0.8rem; background-color:var(--rose); border-color:var(--rose);">Purge</button>` : ''}
                        </div>
                    </td>
                `;
                ...
                tr.innerHTML = `
                    ...
                    <td><span class="badge badge-topic">${p.topic}</span></td>
                    <td><span class="badge badge-platform">${p.platform}</span></td>
                    <td><span class="small-text-gray">${delDate}</span></td>
                    ${ownerHtml}
                    ${actionHtml}
                `;
                
                // Event listener for Restore
                tr.querySelector(".btn-restore-prob").addEventListener("click", async () => {
                    try {
                        await apiRequest(`/api/trash/restore/problem/${p.id}`, { method: 'POST' });
                        showToast("Problem restored successfully!", "success");
                        await loadTrashData();
                        await fetchAllUserData();
                    } catch (err) {
                        showToast(err.message, "error");
                ...
                // Event listener for Purge
                if (isAdmin) {
                    tr.querySelector(".btn-purge-prob").addEventListener("click", () => {
                        showDeleteConfirmation(`Are you sure you want to PERMANENTLY delete "${p.name}"? This cannot be undone.`, async () => {
                            try {
                                await apiRequest(`/api/trash/permanent/problem/${p.id}`, { method: 'DELETE' });
                                showToast("Problem permanently deleted.", "success");
                                await loadTrashData();
                            } catch (err) {
                                showToast(err.message, "error");
                            }
        ...
        // 2. Render deleted plans
        if (!data.plans || data.plans.length === 0) {
            planEmpty.style.display = "block";
            document.getElementById("trashPlansTable").style.display = "none";
        } else {
            planEmpty.style.display = "none";
            document.getElementById("trashPlansTable").style.display = "table";
            
            data.plans.forEach(plan => {
                const tr = document.createElement("tr");
                const delDate = new Date(plan.deleted_at).toLocaleDateString(undefined, {
                    month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit'
                });
                
                let ownerHtml = "";
                if (isAdmin) {
                    ownerHtml = `<td><span class="small-text-gray">${plan.owner_username || 'Unknown'}</span></td>`;
                }
                
                let actionHtml = `
                    <td>
                        <div class="actions-cell">
                            <button class="btn btn-secondary btn-sm btn-restore-plan" data-id="${plan.id}" style="padding: 4px 8px; font-size: 0.8rem;">Restore</button>
                            ${isAdmin ? `<button class="btn btn-primary btn-sm btn-purge-plan" data-id="${plan.id}" style="padding: 4px 8px; font-size: 0.8rem; background-color:var(--rose); border-color:var(--rose);">Purge</button>` : ''}
                        </div>
                    </td>
                `;
                
                tr.innerHTML = `
                    <td><span style="font-weight: 700; border-left: 3px solid ${plan.color}; padding-left: 6px;">${plan.title}</span></td>
                    <td><span style="font-weight: 600;">${plan.target_count} solves</span></td>
                    <td><span class="small-text-gray">${delDate}</span></td>
                    ${ownerHtml}
                    ${actionHtml}
                `;
                
                // Event listener for Restore Plan
                tr.querySelector(".btn-restore-plan").addEventListener("click", async () => {
                    try {
                        await apiRequest(`/api/trash/restore/plan/${plan.id}`, { method: 'POST' });
                        showToast("Study plan restored successfully!", "success");
                        await loadTrashData();
                        await fetchAllUserData();
                    } catch (err) {
                        showToast(err.message, "error");
                ...
                // Event listener for Purge Plan
                if (isAdmin) {
                    tr.querySelector(".btn-purge-plan").addEventListener("click", () => {
                        showDeleteConfirmation(`Are you sure you want to PERMANENTLY delete the plan "${plan.title}"? This cannot be undone.`, async () => {
                            try {
                                await apiRequest(`/api/trash/permanent/plan/${plan.id}`, { method: 'DELETE' });
                                showToast("Study plan permanently deleted.", "success");
                                await loadTrashData();
                            } catch (err) {
                                showToast(err.message, "error");
                            }
        ...
        console.error("Failed to load trash data:", err.message);
    }
}
