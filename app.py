...
def send_otp_message(target, code, purpose):
    ...
    if smtp_user and smtp_password:
        try:
            ...
            server.quit()
            print(f"[SMTP Success] Sent OTP '{code}' to '{target}' for '{purpose}'", flush=True)
            return True
        except Exception as e:
            print(f"[SMTP Error] Failed to send email to {target}: {str(e)}", flush=True)
    else:
        print("[SMTP Warning] SMTP credentials (SMTP_USER/SMTP_PASSWORD) not configured. Falling back to console.", flush=True)
