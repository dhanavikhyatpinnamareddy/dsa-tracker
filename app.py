...
def run_migrations():
    ...
    try:
        ...
        # 2. Problems table migrations
        cursor.execute("ALTER TABLE problems ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN DEFAULT FALSE;")
        cursor.execute("ALTER TABLE problems ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE;")
        cursor.execute("ALTER TABLE problems ADD COLUMN IF NOT EXISTS attachment_url TEXT;")
        
        # 3. Plans table migrations
        cursor.execute("ALTER TABLE plans ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN DEFAULT FALSE;")
        cursor.execute("ALTER TABLE plans ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE;")
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_problems_is_deleted ON problems(is_deleted);")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_plans_is_deleted ON plans(is_deleted);")
...
def get_problems():
    ...
    # Retrieve all user problems that are NOT soft deleted
    cursor.execute("SELECT * FROM problems WHERE user_id = %s AND is_deleted = FALSE ORDER BY date DESC, id DESC", (user['id'],))
    problem_rows = cursor.fetchall()
    
    # Retrieve mappings of problems to study plans (exclude deleted plans)
    cursor.execute('''
...
def save_problem():
    ...
    if pid:
        ...
        cursor.execute("SELECT * FROM problems WHERE id = %s AND user_id = %s AND is_deleted = FALSE", (pid, user['id']))
        if not cursor.fetchone():
        ...
        # Check if the same problem already exists and is not soft deleted
        cursor.execute("""
            SELECT id
            FROM problems
            WHERE user_id = %s
        ...
        cursor.execute("""
            ...
            (id, user_id, name, difficulty, platform, topic, date, url, notes, is_deleted, attachment_url)
    ...
    # Insert checked plan mappings (making sure they belong to current user and are not soft deleted)
    for plan_id in associated_plans:
        cursor.execute("SELECT id FROM plans WHERE id = %s AND user_id = %s AND is_deleted = FALSE", (plan_id, user['id']))
        if cursor.fetchone():
            cursor.execute(
...
def delete_problem(pid):
    ...
    cursor.execute("UPDATE problems SET is_deleted = TRUE, deleted_at = CURRENT_TIMESTAMP WHERE id = %s", (pid,))
    db.commit()
...
def bulk_delete_problems():
    ...
    # Validate all IDs belong to the user and are not already soft-deleted
    placeholders = ','.join('%s' for _ in ids)
    cursor.execute(
        f"SELECT id FROM problems WHERE id IN ({placeholders}) AND user_id = %s AND is_deleted = FALSE",
        (*ids, user['id'])
    )
    ...
    cursor.execute(
        f"UPDATE problems SET is_deleted = TRUE, deleted_at = CURRENT_TIMESTAMP WHERE id IN ({valid_placeholders})", 
        valid_ids
    )
    db.commit()
    
    return jsonify({'message': f'Successfully soft-deleted {len(valid_ids)} problem logs.'})
...
def get_plans():
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT * FROM plans WHERE user_id = %s AND is_deleted = FALSE ORDER BY id DESC", (user['id'],))
...
def save_plan():
    ...
    cursor.execute('''
        INSERT INTO plans (id, user_id, title, description, target_count, color, is_deleted)
...
def delete_plan(plan_id):
    ...
    cursor.execute("SELECT * FROM plans WHERE id = %s AND user_id = %s AND is_deleted = FALSE", (plan_id, user['id']))
    if not cursor.fetchone():
        return jsonify({'error': 'Study plan not found or unauthorized.'}), 404
        
    # Soft delete instead of hard delete
    cursor.execute("UPDATE plans SET is_deleted = TRUE, deleted_at = CURRENT_TIMESTAMP WHERE id = %s", (plan_id,))
    db.commit()
...
def reset_progress():
    ...
    # Soft delete resets to prevent data loss
    if scope == "problems":
        cursor.execute("UPDATE problems SET is_deleted = TRUE, deleted_at = CURRENT_TIMESTAMP WHERE user_id = %s", (user['id'],))
        db.commit()
        return jsonify({'message': 'All active problem logs soft-deleted successfully.'})
    elif scope == "plans":
        cursor.execute("UPDATE plans SET is_deleted = TRUE, deleted_at = CURRENT_TIMESTAMP WHERE user_id = %s", (user['id'],))
        db.commit()
...
def get_trash():
    ...
    if user['is_admin']:
        ...
        cursor.execute('''
            ...
            WHERE pl.is_deleted = TRUE 
            ORDER BY pl.deleted_at DESC
        ''')
        plans = cursor.fetchall()
    else:
        # Regular user views their own soft-deleted items
        cursor.execute("SELECT * FROM problems WHERE user_id = %s AND is_deleted = TRUE ORDER BY deleted_at DESC", (user['id'],))
        problems = cursor.fetchall()
        
        cursor.execute("SELECT * FROM plans WHERE user_id = %s AND is_deleted = TRUE ORDER BY deleted_at DESC", (user['id'],))
        plans = cursor.fetchall()
...
def restore_trash(item_type, item_id):
    ...
    db = get_db()
    cursor = db.cursor()
    table = 'problems' if item_type == 'problem' else 'plans'
    
    # Verify ownership or admin privileges
    if user['is_admin']:
        cursor.execute(f"SELECT id FROM {table} WHERE id = %s AND is_deleted = TRUE", (item_id,))
    else:
        cursor.execute(f"SELECT id FROM {table} WHERE id = %s AND user_id = %s AND is_deleted = TRUE", (item_id, user['id']))
        
    if not cursor.fetchone():
        return jsonify({'error': 'Soft-deleted item not found.'}), 404
        
    cursor.execute(f"UPDATE {table} SET is_deleted = FALSE, deleted_at = NULL WHERE id = %s", (item_id,))
    db.commit()
...
def get_admin_stats():
    ...
    cursor.execute("SELECT COUNT(*) as count FROM problems WHERE is_deleted = FALSE")
    total_problems = cursor.fetchone()['count']
    
    # 3. User lists with registration date and active solve totals
    cursor.execute('''
        SELECT u.username, u.created_at, u.is_admin, COUNT(p.id) FILTER (WHERE p.is_deleted = FALSE) as solved_count
        FROM users u
